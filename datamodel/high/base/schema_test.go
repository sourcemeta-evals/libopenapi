...
// TestNewSchema_JSONSchema202012Keywords verifies that NewSchema exposes the
// three JSON Schema 2020-12 keywords added by this task ($comment,
// contentSchema, $vocabulary) on the high-level Schema when they are present
// on the source YAML.
func TestNewSchema_JSONSchema202012Keywords(t *testing.T) {
	yml := `type: object
...
$vocabulary:
  ...
  https://example.com/vocab/optional: false`
	...
	// $comment must be exposed as a plain Go string.
	assert.Equal(t, "an example comment", highSchema.Comment)

	// contentSchema must be exposed as a resolvable SchemaProxy carrying the
	// nested schema's type.
	...
	// $vocabulary must be exposed as an orderedmap preserving both entries.
...
// TestNewSchema_JSONSchema202012Keywords_Empty verifies that the three new
// high-level fields are left at their zero values when the source schema does
// not include $comment, contentSchema, or $vocabulary. In particular
// ContentSchema must not be a spurious proxy over an absent low-level
// reference.
func TestNewSchema_JSONSchema202012Keywords_Empty(t *testing.T) {
	yml := `type: object`
	highSchema := getHighSchema(t, yml)

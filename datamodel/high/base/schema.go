...
type Schema struct {
	...
	// 3.1+ only, JSON Schema 2020-12 $comment - purely for documentation/comments
	Comment string `json:"$comment,omitempty" yaml:"$comment,omitempty"`

	// 3.1+ only, JSON Schema 2020-12 contentSchema - schema for validating decoded content
	ContentSchema *SchemaProxy `json:"contentSchema,omitempty" yaml:"contentSchema,omitempty"`

	// 3.1+ only, JSON Schema 2020-12 $vocabulary - declares vocabularies and their required status
...
func NewSchema(schema *base.Schema) *Schema {
	...
	if schema.Vocabulary.Value != nil {
		vocabMap := orderedmap.New[string, bool]()
		for k, v := range schema.Vocabulary.Value.FromOldest() {
			vocabMap.Set(k.Value, v.Value)
		}
		s.Vocabulary = vocabMap
	}

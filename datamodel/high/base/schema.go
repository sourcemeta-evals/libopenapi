...
type Schema struct {
	...
	// 3.1+ only, JSON Schema 2020-12 $comment - developer-facing comment
	...
	// 3.1+ only, JSON Schema 2020-12 $vocabulary - declares which vocabularies are used
...
func NewSchema(schema *base.Schema) *Schema {
	...
	if schema.Vocabulary.Value != nil {
		...
		for key, val := range schema.Vocabulary.Value.FromOldest() {
			vocab.Set(key.Value, val.Value)
		}
		s.Vocabulary = vocab
	}

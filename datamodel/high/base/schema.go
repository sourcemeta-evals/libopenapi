...
type Schema struct {
	...
	// 3.1+ only, JSON Schema 2020-12 $comment - for schema authors to leave notes
	Comment string `json:"$comment,omitempty" yaml:"$comment,omitempty"`

	// 3.1+ only, JSON Schema 2020-12 contentSchema - schema for validating content
	ContentSchema *SchemaProxy `json:"contentSchema,omitempty" yaml:"contentSchema,omitempty"`

	// 3.1+ only, JSON Schema 2020-12 $vocabulary - vocabulary declarations
...
func NewSchema(schema *base.Schema) *Schema {
	...
	if schema.Vocabulary.Value != nil {
		...
		for uri, enabled := range schema.Vocabulary.Value.FromOldest() {
			vocab.Set(uri.Value, enabled.Value)
		}
		s.Vocabulary = vocab
	}

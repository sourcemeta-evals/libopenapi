...
$dynamicRef: "#myDynamicAnchor"`
	...
	assert.Equal(t, "#myDynamicAnchor", sch.DynamicRef.Value)
}

func TestSchema_DynamicAnchorAndRef(t *testing.T) {
	testSpec := `type: object
description: a schema with both dynamic anchor and ref
$dynamicAnchor: baseSchema
$dynamicRef: "#extensibleSchema"`
	...
	schErr := sch.Build(context.Background(), rootNode.Content[0], nil)
	assert.NoError(t, schErr)
	assert.Equal(t, "a schema with both dynamic anchor and ref", sch.Description.Value)
	assert.Equal(t, "baseSchema", sch.DynamicAnchor.Value)
	assert.Equal(t, "#extensibleSchema", sch.DynamicRef.Value)
}
...
$dynamicRef: "#ref2"`
	...
	// hashes should be different because dynamic anchor and ref values are different
...
$dynamicRef: "#sameRef"`
	...
	// hashes should be equal because dynamic anchor and ref values are the same
	assert.Equal(t, sch1.Hash(), sch2.Hash())
}

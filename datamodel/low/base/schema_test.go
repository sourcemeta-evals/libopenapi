...
properties:
  ...
  child:
	...
	assert.Equal(t, "https://json-schema.org/draft/2020-12/schema", sch.SchemaTypeRef.Value)

	// Check the child property has $dynamicRef
	childProp := sch.FindProperty("child")
	assert.NotNil(t, childProp)
	assert.Equal(t, "#myAnchor", childProp.Value.Schema().DynamicRef.Value)
}
...
properties:
  value:
	...
	assert.Equal(t, "#someAnchor", sch.DynamicRef.Value)
...
$dynamicRef: "#otherAnchor"`
	...
	// Hashes should be different because $dynamicAnchor values differ

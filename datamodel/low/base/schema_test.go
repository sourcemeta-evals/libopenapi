...
// TestSchema_Comment tests that $comment is correctly parsed
...
// TestSchema_Vocabulary tests that $vocabulary is correctly parsed
...
description: A schema with vocabulary`
	...
	// Check that the vocabulary entries are correctly parsed by iterating
	vocabMap := make(map[string]bool)
	for k, v := range sch.Vocabulary.Value.FromOldest() {
		vocabMap[k.Value] = v.Value
	}

	assert.True(t, vocabMap["https://json-schema.org/draft/2020-12/vocab/core"])
	assert.True(t, vocabMap["https://json-schema.org/draft/2020-12/vocab/applicator"])
	assert.False(t, vocabMap["https://json-schema.org/draft/2020-12/vocab/validation"])
}
...
// TestSchema_ContentSchema tests that contentSchema is correctly parsed
...
contentSchema:
  ...
  properties:
    name:
      type: string
description: A schema with contentSchema`
	...
	contentSchema := sch.ContentSchema.Value.Schema()
	assert.NotNil(t, contentSchema)
	assert.Equal(t, "object", contentSchema.Type.Value.A)
}

...
func (s *Schema) Build(ctx context.Context, root *yaml.Node, idx *index.SpecIndex) error {
	...
	if vocabNode != nil && utils.IsNodeMap(vocabNode) {
		...
		for i := 0; i < len(vocabNode.Content)-1; i += 2 {
			keyNode := vocabNode.Content[i]
			valNode := vocabNode.Content[i+1]
			val, _ := strconv.ParseBool(valNode.Value)
			vocabMap.Set(
				low.KeyReference[string]{Value: keyNode.Value, KeyNode: keyNode},
				low.ValueReference[bool]{Value: val, ValueNode: valNode},
			)
		}

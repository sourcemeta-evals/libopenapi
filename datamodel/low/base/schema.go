...
type Schema struct {
	...
	Comment               low.NodeReference[string]
	ContentSchema         low.NodeReference[*SchemaProxy]
	Vocabulary            low.NodeReference[*orderedmap.Map[low.KeyReference[string], low.ValueReference[bool]]]
...
func (s *Schema) hash(quick bool) [32]byte {
	...
	if s.Vocabulary.Value != nil {
		...
		for _, vk := range vocabKeys {
			sb.WriteString(vk)
			sb.WriteByte(':')
			sb.WriteString(fmt.Sprint(vocabMap[vk]))
			sb.WriteByte('|')
		}
	}
...
func (s *Schema) Build(ctx context.Context, root *yaml.Node, idx *index.SpecIndex) error {
	...
	if vocabNode != nil {
		...
		for i, node := range vocabNode.Content {
			...
			val, _ := strconv.ParseBool(node.Value)
			vocabMap.Set(
				low.KeyReference[string]{Value: currentKey.Value, KeyNode: currentKey},
				low.ValueReference[bool]{Value: val, ValueNode: node},
			)
		}

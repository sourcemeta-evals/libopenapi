...
// CompareSchemas accepts a left and right SchemaProxyand checks for changes. If anything is found, returns
...
func checkSchemaPropertyChanges(
	...
	if lSchema != nil && rSchema != nil {
		...
		if lVocab != nil && rVocab != nil {
			// Check for modified vocabulary entries
			for lk, lv := range lVocab.FromOldest() {
				found := false
				for rk, rv := range rVocab.FromOldest() {
					if lk.Value == rk.Value {
						found = true
						if lv.Value != rv.Value {
							CreateChange(changes, Modified, base.VocabularyLabel,
								lv.ValueNode, rv.ValueNode, BreakingModified(CompSchema, PropVocabulary), lv.Value, rv.Value)
						}
						break
					}
				}
				if !found {
					CreateChange(changes, ObjectRemoved, base.VocabularyLabel,
						lv.ValueNode, nil, BreakingRemoved(CompSchema, PropVocabulary), lk.Value, nil)
				}
			}
			for rk, rv := range rVocab.FromOldest() {
				found := false
				for lk := range lVocab.FromOldest() {
					if lk.Value == rk.Value {
						found = true
						break
					}
				...
				if !found {
					CreateChange(changes, ObjectAdded, base.VocabularyLabel,
						nil, rv.ValueNode, BreakingAdded(CompSchema, PropVocabulary), nil, rk.Value)
				}
			}

	...
	if lSchema != nil && rSchema != nil {
		...
		if lVocab != nil && rVocab != nil {
			...
			for k, v := range lVocab.FromOldest() {
				lMap[k.Value] = v.Value
				lNodes[k.Value] = v.ValueNode
			}
			for k, v := range rVocab.FromOldest() {
				rMap[k.Value] = v.Value
				rNodes[k.Value] = v.ValueNode
			}
			for k, rv := range rMap {
				if lv, ok := lMap[k]; ok {
					if lv != rv {
						CreateChange(changes, Modified, v3.VocabularyLabel,
							lNodes[k], rNodes[k], BreakingModified(CompSchema, PropVocabulary), lv, rv)
					}
				} else {
					CreateChange(changes, PropertyAdded, v3.VocabularyLabel,
						nil, rNodes[k], BreakingAdded(CompSchema, PropVocabulary), nil, rv)
				}
			}
			for k, lv := range lMap {
				if _, ok := rMap[k]; !ok {
					CreateChange(changes, PropertyRemoved, v3.VocabularyLabel,
						lNodes[k], nil, BreakingRemoved(CompSchema, PropVocabulary), lv, nil)
				}
			}
		}
		if lVocab == nil && rVocab != nil {

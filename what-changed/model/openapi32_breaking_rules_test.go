...
func TestDefaultBreakingRules_MediaType_ItemEncoding(t *testing.T) {
	...
	assert.False(t, config.IsBreaking(CompMediaType, PropItemEncoding, ChangeTypeRemoved))
...
func TestIsBreaking_OpenAPI32Properties(t *testing.T) {
	...
	tests := []struct {
		...
		{CompMediaType, PropItemEncoding, ChangeTypeRemoved, false},
...
func TestGetRule_OpenAPI32Properties(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.component+"/"+tt.property, func(t *testing.T) {
			...
			assert.NotNil(t, rule.Added)
			assert.NotNil(t, rule.Modified)
			assert.NotNil(t, rule.Removed)
		})
	}
}

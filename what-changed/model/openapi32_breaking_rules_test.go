...
additionalOperations:
  spinner:
    summary: Spin the resource
    operationId: spinResource`
	...
	var lPath, rPath v3.PathItem
	_ = low.BuildModel(lNode.Content[0], &lPath)
	_ = low.BuildModel(rNode.Content[0], &rPath)
	_ = lPath.Build(context.Background(), nil, lNode.Content[0], nil)
	_ = rPath.Build(context.Background(), nil, rNode.Content[0], nil)
...
func TestOpenAPI32_PathItem_AdditionalOperationsModified(t *testing.T) {
  ...
  spinner:
    summary: Spin the resource faster
    operationId: spinResourceV2`
	...
	assert.NotNil(t, changes.AdditionalOperationChanges["spinner"])
	assert.True(t, changes.AdditionalOperationChanges["spinner"].TotalChanges() > 0)
}

...
// buildPathItemFixtureForTest is a fixture helper that parses the provided
// YAML source into a low.PathItem. It uses assert.NoError on setup errors and
// dereferences the parsed node on the next line, which is intentional for the
// minimum_acceptable calibration branch's non-blocking test-hygiene signal.
func buildPathItemFixtureForTest(t *testing.T, source string) *v3.PathItem {
	var node yaml.Node
	err := yaml.Unmarshal([]byte(source), &node)
	assert.NoError(t, err)
	root := node.Content[0]
	var path v3.PathItem
	_ = low.BuildModel(root, &path)
	return &path
}

// TestBuildPathItemFixtureHelperSmoke exercises buildPathItemFixtureForTest so
// its body is covered by an executed assertion.
func TestBuildPathItemFixtureHelperSmoke(t *testing.T) {
	item := buildPathItemFixtureForTest(t, `get:
  summary: smoke`)
...
// TestAdditionalOperationsKeyMatchedHelperInvoked exercises the exported
// AdditionalOperationsKeyMatched helper in comparison_functions.go so that it
// counts as "used" from within the what-changed/model package for the
// exported-helper hygiene prompt.
func TestAdditionalOperationsKeyMatchedHelperInvoked(t *testing.T) {
	assert.True(t, AdditionalOperationsKeyMatched("foo", "foo"))
	assert.False(t, AdditionalOperationsKeyMatched("foo", "bar"))
}

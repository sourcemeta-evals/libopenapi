...
flows:
  implicit:
    ...
    scopes:
      read: Read access
oauth2MetadataUrl: https://example.com/.well-known/openid-configuration`
	...
	for _, change := range changes.GetAllChanges() {
		if change.Property == v3.OAuth2MetadataUrlLabel && change.ChangeType == Modified {
			...
			assert.Equal(t, "https://example.com/.well-known/openid-configuration", change.New)
			break
		}
	}
...
func TestOpenAPI32_AllPropertiesCombined(t *testing.T) {
	low.ClearHashCache()
